...
registryTrackerActivityCheckerAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#registryTrackerActivityCheckerAddress}
...
echo "$(jq '. += {"registryTrackerActivityCheckerAddress":"'$registryTrackerActivityCheckerAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$registryTrackerActivityCheckerAddress $contractPath --constructor-args $(cast abi-encode "constructor(address)" $constructorArgs)"
