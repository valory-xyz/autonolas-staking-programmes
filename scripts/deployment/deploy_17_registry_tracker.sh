...
stakingFactoryAddress=$(jq -r '.stakingFactoryAddress' globals.json)
...
echo "$(jq '. += {"registryTrackerAddress":"'$registryTrackerAddress'"}' globals.json)" > globals.json
