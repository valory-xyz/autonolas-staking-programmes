...
stakingNativeTokenAddress=$(jq -r '.stakingNativeTokenAddress' $globals)
...
# Encode the initialize function call for StakingNativeToken
# initialize((bytes32,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256[],uint256,bytes32,bytes32,address,address))
initPayload=$(cast calldata "initialize((bytes32,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256[],uint256,bytes32,bytes32,address,address))" "($metadataHash,$maxNumServices,$rewardsPerSecond,$minStakingDeposit,$minNumStakingPeriods,$maxNumInactivityPeriods,$livenessPeriod,$timeForEmissions,$numAgentInstances,$agentIds,$threshold,$configHash,$proxyHash,$serviceRegistry,$activityChecker)")
...
echo "Deploying StakingNativeTokenInstance via StakingFactory"

# Call createStakingInstance on the factory
execCmd="cast send --rpc-url $networkURL$API_KEY $walletArgs $stakingFactoryAddress \"createStakingInstance(address,bytes)\" $stakingNativeTokenAddress $initPayload"
...
stakingNativeTokenInstanceAddress=$(cast receipt --rpc-url "$networkURL$API_KEY" "$txHash" --json | jq -r '.logs[0].topics[2]' | sed 's/0x000000000000000000000000/0x/')

# Get output length
outputLength=${#stakingNativeTokenInstanceAddress}
...
echo "$(jq '. += {"stakingNativeTokenInstanceAddress":"'$stakingNativeTokenInstanceAddress'"}' $globals)" > $globals

echo "StakingNativeTokenInstance deployed at: $stakingNativeTokenInstanceAddress"
