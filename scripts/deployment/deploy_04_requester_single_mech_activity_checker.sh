...
agentMechAddress=$(jq -r '.agentMechAddress' $globals)
...
requesterSingleMechActivityCheckerAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#requesterSingleMechActivityCheckerAddress}
...
echo "$(jq '. += {"requesterSingleMechActivityCheckerAddress":"'$requesterSingleMechActivityCheckerAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$requesterSingleMechActivityCheckerAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,uint256)" $constructorArgs)"
...
echo "$contractName deployed at: $requesterSingleMechActivityCheckerAddress"
