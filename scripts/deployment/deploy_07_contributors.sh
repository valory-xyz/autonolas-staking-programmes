...
constructorArgs="$serviceManagerProxyAddress $olasAddress $stakingFactoryAddress $agentId $configHash"
...
contributorsAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#contributorsAddress}
...
echo "$(jq '. += {"contributorsAddress":"'$contributorsAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$contributorsAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address,uint256,bytes32)" $constructorArgs)"
